...
function createTable(/*colorArr,*/ n, m) {
  let k = 0;
  let table = "<table border='10' width = 700px height = 700px align='center'>"
    let arr = []
    const randNum = Math.floor(Math.random() * (n*m))
    for (let i = 0; i < randNum; i++) {
      arr.push(rand75())
    }
    const available = (n*m) - randNum;
    for (let i = 0; i < available; i++) {
      arr.push(2)
    }
...
app.get("/", (req, res) => {
  let colors = createTable();
  shuffle(colors)
  let board = createTable(/*colors,*/ 10, 13)
  res.send(board)
});
app.listen(port, err => {
  if (err) {
